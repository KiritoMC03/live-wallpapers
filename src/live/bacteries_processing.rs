...
pub const ALIVE_TIME : Range<f32> = 0.0..100.0;
...
pub const CARNIVORE_RATE : f32 = 5.0;
pub const CARNIVORE_DAMAGE : f32 = 10.0;
...
fn process_carnivore(app: &mut AppData, a: usize, b: usize) {
    ...
    bac.left_time[a] -= CARNIVORE_DAMAGE * (cav_b - cav_a).clamp(0.0, f32::MAX);
    bac.left_time[b] -= CARNIVORE_DAMAGE * (cav_a - cav_b).clamp(0.0, f32::MAX);

    bac.energy[a] += CARNIVORE_RATE * cav_a - CARNIVORE_COST * cav_a;
    bac.energy[b] += CARNIVORE_RATE * cav_b - CARNIVORE_COST * cav_b;
}
